...
function pickOWAir(ow) {
  ...
  if (!item)
    return { aqi: null, components: null, dt: null, date: null, weekday: null };
...
export async function getCityUID(req, res) {
  try {
    ...
    if (!Number.isInteger(uid))
      return res.status(400).json({ error: "uid must be integer" });
...
export async function getGeo(req, res) {
  try {
    ...
    if (!WAQI_TOKEN)
      return res
        .status(500)
        .json({ success: false, error: "Missing WAQI_TOKEN" });
    if (!OW_API_KEY)
      return res
        .status(500)
        .json({ success: false, error: "Missing OPEN_WEATHER_API_KEY" });
    ...
    if (!Number.isFinite(latNum) || !Number.isFinite(lonNum)) {
      return res
        .status(400)
        .json({ success: false, error: "lat & lon required (numbers)" });
    }
    ...
    if (waqiResp.status === "fulfilled") {
      if (waqiResp.value?.status === "ok") {
        ...
        waqiErr =
          waqiResp.value?.data || waqiResp.value?.status || "WAQI error";
      }
    } else {
...
export async function getHistory(req, res) {
  try {
    ...
    if (!TOKEN)
      return res
        .status(500)
        .json({ success: false, message: "Missing OPEN_WEATHER_API_KEY" });
